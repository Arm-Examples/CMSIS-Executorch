...
### FVP run/debug is not wired up

This is the honest gap. The scripts behind the CMSIS Solution panel's
**Load / Run / Debug** buttons are POSIX shell:

- `.vscode/start-fvp-gdb.sh`
- `.vscode/stop-fvp-gdb.sh`
- `.vscode/tail-fvp-log.sh`

On Windows they need Git Bash on `PATH`. Building, exporting, and running the
FVP directly from a command line are all unaffected:
...
Porting those three scripts to PowerShell (or folding them into Python, per the
rule above) is the natural next step for this branch.
